...
class GameWindow(arcade.Window):
    ...
    #def on_update(self):
        ...
        #print(f"self.master_status_name   {self.master_status_name}")

        
        if self.master_status_name != "leverMid": #leverMid is electricity OFF

        # For each moving sprite, see if we've reached a boundary and need to
        # reverse course.
            for moving_sprite in self.mechanics_list:
                ...
                #print("................................................................")
                #print(f"boundary_right    {moving_sprite.boundary_right}     boundary_left   {moving_sprite.boundary_left}      boundary_top  {moving_sprite.boundary_top}        boundary_bottom {moving_sprite.boundary_bottom}   change_x {moving_sprite.change_x}   change_y {moving_sprite.change_y}")
        ...
        elif self.master_status_name == "leverMid":
            for moving_sprite in self.mechanics_list:
        ...
        #for coin in self.coins_list:
            ...
            #coin_hit_list = arcade.check_for_collision_with_list(coin, self.player_sprite)
            #coin_hit_list = arcade.check_for_collision_with_list(coin, self.player_list)
        coin_hit_list = arcade.check_for_collision_with_list(self.player_list[0], self.coins_list)


        #print(f"___________________________________________________________________________________________********************coin_hit_list {coin_hit_list}")


        for coin in coin_hit_list:

            print(f"score before updating:   {self.score}")



            print(f"touching   {coin}")

            self.score += 1
            print(f"score updated = {self.score},   coin is being to be removed ...")
            

            #self.physics_engine.remove_sprite(coin)

            coin.remove_from_sprite_lists()

            print(f"coin {coin}")
            print(f"coin_hit_list {coin_hit_list}")





        key_lock_door_HIT_list = arcade.check_for_collision_with_list(self.player_list[0], self.key_lock_door_list)
        if key_lock_door_HIT_list != []:
            
            just_hited = key_lock_door_HIT_list[0]
            just_hited_name = just_hited.properties["name"]


            #self.crash_debug(key_lock_door_HIT_list)
            #self.crash_debug(key_lock_door_HIT_list[0].properties["name"])

            #if key_lock_door_HIT_list[0].properties["name"] == "key_yellow":
            if just_hited_name == "key_yellow":
                self.inventory.update(set([just_hited_name]))
                just_hited.remove_from_sprite_lists()


            if just_hited_name == "lock_yellow":
                if "key_yellow" in  self.inventory:
                    just_hited.remove_from_sprite_lists()
                    self.inventory.remove("key_yellow")

                    for kld in self.key_lock_door_list:
                        if kld.properties["name"] == "door_yellow":
                            kld.remove_from_sprite_lists()



        trap_hit_list = arcade.check_for_collision_with_list(self.player_list[0], self.trap_list)
        #trap_hit_list = arcade.check_for_collision_with_list(self.trap_list, self.player_list[0])
        
        if trap_hit_list != []:
            just_hited = trap_hit_list[0]
            print(f"lives before updating:   {self.lives}")
            print(f"touching   {just_hited}")
            self.lives -= 1
            print(f"lives updated = {self.lives},   respawning or die ...")

            if self.lives >0:
                #self.crash_debug("respawn")
                print("...")
                self.spawn_player()
                print(".....")
    ...
    def on_draw(self):
        ...
        remaining_lives_text = f";-)  ;-)   ;-)    Lives: {self.lives}"
        arcade.draw_text(remaining_lives_text, 80, 900, arcade.csscolor.PINK, 30)
